...
%% @hidden
%%
mtp3_user_cause(<<Cause:16, User:16>>) ->
	{mtp3_user(User), mtp3_cause(Cause)};
mtp3_user_cause({User, Cause}) ->
	U = mtp3_user(User),
	C = mtp3_cause(Cause),
	<<C:16, U:16>>.
...
error_code(<<5:32>>) -> unsupported_traffic_mode_type;
...
error_code(<<17:32>>) -> invalid_parameter_value;
error_code(<<18:32>>) -> parameter_field_error;
error_code(<<19:32>>) -> unexpected_parameter;
error_code(<<20:32>>) -> destination_status_unknown;
error_code(<<21:32>>) -> invalid_network_appearance;
error_code(<<22:32>>) -> missing_parameter;
error_code(<<25:32>>) -> invalid_routing_context;
error_code(<<26:32>>) -> no_configured_as_for_asp;
error_code(invalid_version) -> <<1:32>>;
error_code(unsupported_message_class) -> <<3:32>>;
error_code(unsupported_message_type) -> <<4:32>>;
error_code(unsupported_traffic_mode_type) -> <<5:32>>;
...
error_code(invalid_parameter_value) -> <<17:32>>;
error_code(parameter_field_error) -> <<18:32>>;
error_code(unexpected_parameter) -> <<19:32>>;
error_code(destination_status_unknown) -> <<20:32>>;
